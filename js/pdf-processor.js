...
// ⚡ Bolt: High-performance Uint8Array to binary string conversion.
// Avoids the massive overhead of Array.from(bytes, ...).join('') which creates
// millions of intermediate array elements and executes closures for every byte.
// The chunked approach prevents "Maximum call stack size exceeded" errors.
...
async function _substituirViaBytesRaw(pdfBytes, specs, specsHex = []) {
  ...
  let bin = bytesToBinaryString(bytes);
  ...
  while ((match = regexStreams.exec(bin)) !== null) {
    ...
    const newBin = bytesToBinaryString(pako.deflate(encodeLatin1(result.text)));
