...
// Otimização: Evita a criação de arrays intermediários grandes
function bytesToStringChunked(bytes) {
  let str = '';
  const chunk = 32768; // Limite seguro para não estourar a pilha de chamadas
...
async function _substituirViaBytesRaw(pdfBytes, specs, specsHex = []) {
  ...
  let bin = bytesToStringChunked(bytes);
  ...
  while ((match = regexStreams.exec(bin)) !== null) {
    ...
    const newBin = bytesToStringChunked(pako.deflate(encodeLatin1(result.text)));
...
function coletarTextosDecodificadosViaBytes(pdfBytes) {
  const bytes = toUint8Array(pdfBytes);
  const bin = bytesToStringChunked(bytes);
  const textos = [];
  const regexStreams = /stream\r?\n([\s\S]*?)\r?\nendstream/g;
  let match;
