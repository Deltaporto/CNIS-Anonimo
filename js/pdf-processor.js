...
// ⚡ Bolt Optimization: Chunked stringification is significantly faster than Array.from().join('')
// for large Uint8Arrays (like raw PDF bytes or inflated streams), bypassing call stack limits.
function bytesToBinaryString(bytes) {
  let str = '';
  const maxChunk = 32768;
  for (let i = 0; i < bytes.length; i += maxChunk) {
    str += String.fromCharCode.apply(null, bytes.subarray(i, i + maxChunk));
  }
  return str;
}
