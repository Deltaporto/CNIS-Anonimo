...
function _sobrepoeRangeProtegido(match, ranges) {
  ...
  // ⚡ Bolt: Fast-path array scanning. Replaced Array.prototype.some with a
  // traditional for loop to avoid callback allocation overhead in this highly
  // executed hot path during string redaction overlap checks.
...
function contarAchados(texto) {
  ...
  // ⚡ Bolt: Cache protected ranges outside the filter loops.
  // Recalculating _coletarRangesProtegidos(texto) for every matched identifier
  // causes severe O(N*M) performance degradation.
  const rangesProtegidos = _coletarRangesProtegidos(texto);
  const identificadores = [...texto.matchAll(_globalizar(IDENTIFICADOR_LONGO_PATTERN))]
    .filter(m => !_sobrepoeRangeProtegido(m, rangesProtegidos) && !/^0+$/.test(m[0]))
    .length;
