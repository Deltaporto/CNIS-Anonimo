...
## 2025-02-14 - Visual styling for keyboard shortcut hints
**Learning:** Unstyled `<kbd>` tags inside buttons blend in with normal text, making keyboard shortcut hints visually ambiguous and easy to miss.
**Action:** Establish a visual design system pattern for `<kbd>` tags by applying explicit styling (e.g., borders, bottom-heavy weight, monospace fonts) to simulate a physical keycap, and apply `inline-flex` and `gap` to parent buttons for proper alignment and spacing alongside text.
