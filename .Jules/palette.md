...
## 2024-05-18 - Progress Indicator Accessibility
**Learning:** For custom-built progress indicators using generic HTML tags (e.g., div), it's not enough to just visually update the width. Screen readers need context to announce the progress.
**Action:** Explicitly set `role="progressbar"`, `aria-valuemin`, `aria-valuemax`, and an `aria-label` on the wrapper element, and dynamically update `aria-valuenow` via JavaScript as the progress changes to ensure accurate screen reader announcements.